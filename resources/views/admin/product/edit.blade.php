...
<div class="container">
    ...
    <form action="{{ route('admin.product.update', $viewData['product']->getId()) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="category_id" class="form-control" required>
                @foreach ($viewData['categories'] as $category)
                    <option value="{{ $category->getId() }}" @if($category->getId() == $viewData['product']->getCategory()->getId()) selected @endif>
                        {{ $category->getName() }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <img src="{{asset('storage/' . $viewData['product']->getImagePath()) }}" class="img-fluid mt-2" style="max-width: 200px;">
        </div>
